...
@loader.tds
class deepseek_module(loader.Module):
    """Модуль для общения с Deepseek AI"""
    ...
    async def deepseekcmd(self, message):
        """<reply to photo / text> — отправить запрос к Deepseek.
        Отредактировано @Hikimuro"""
        if not self.config["api_key"]:
            await message.edit(
                "<emoji document_id=5274099962655816924>❗️</emoji> <b>API ключ не указан. Получи его и укажи в конфиге</b>"
            )
            return
        ...
        edit_message = (
            ...
            if prompt else
            "<emoji document_id=5443038326535759644>💬</emoji> <b>Ответ от Deepseek (по изображению):</b> <emoji document_id=4988080790286894217>🫥</emoji>"
        )
